...
def test(img_dir, test_list):
    ...
    img_input = tf.compat.v1.placeholder(tf.float32, shape=[1, IMG_SIZE, IMG_SIZE, 3])
   ...
   # Use a variable scope to control variable reuse
    ...
    checkpoint_file ='/content/drive/MyDrive/DS-store/Det_train/pretrained/Checkpoint-500'
    ...
    with tf.compat.v1.Session(config=cfg) as sess:
        ...
        for fn in data_list:
            ...
            if (boxes is not None) and (len(boxes[0]) > 0):
                print(boxes)
                cv2.polylines(bgr_input, np.int32(boxes), True, (0, 255, 0),3)
                cv2.imwrite('/content/drive/MyDrive/DS-store/detection/detect_text/test_detection' + fn[:len(fn) - 4] + '_img.jpg', bgr_input)


if __name__ == '__main__':
    test('/content/drive/MyDrive/DS-store/test', 
         '/content/drive/MyDrive/DS-store/Det_train/test.list')
